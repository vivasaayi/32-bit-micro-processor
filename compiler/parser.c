...
int is_at_end(Parser *parser) { return peek(parser)->type == TOK_EOF; }
...
AstNode *parse(TokenList *tokens) {
  Parser parser = {.tokens = tokens, .current = 0, .had_error = 0};
...
AstNode *parse_program(Parser *parser) {
  ...
  while (!is_at_end(parser)) {
    ...
    if (decl) {
      ...
      // If no declaration was parsed and no error was set, break to avoid
      // infinite loop
      break;
    }
  }
...
AstNode *parse_declaration(Parser *parser) {
  ...
  if (check(parser, TOK_INT) || check(parser, TOK_CHAR) ||
      check(parser, TOK_FLOAT) || check(parser, TOK_DOUBLE) ||
      check(parser, TOK_VOID)) {
...
AstNode *parse_variable_declaration(Parser *parser) {
  ...
  if (match(parser, TOK_LBRACKET)) {
    ...
    // Create array type (size will be determined later if 0 and there's an
    // initializer)
...
AstNode *parse_struct_declaration(Parser *parser) {
  ...
  while (brace_count > 0 && !is_at_end(parser)) {
    if (peek(parser)->type == TOK_LBRACE)
      brace_count++;
    if (peek(parser)->type == TOK_RBRACE)
      brace_count--;
    advance(parser);
  }
...
AstNode *parse_statement(Parser *parser) {
  ...
  if (check(parser, TOK_INT) || check(parser, TOK_CHAR) ||
      check(parser, TOK_FLOAT) || check(parser, TOK_DOUBLE) ||
      check(parser, TOK_VOID) || check(parser, TOK_BOOL)) {
    return parse_variable_declaration(parser);
  }
...
AstNode *parse_for_statement(Parser *parser) {
  ...
  // Create placeholder for statement - in real implementation would parse init,
  // condition, increment
...
AstNode *parse_switch_statement(Parser *parser) {
  ...
  while (!is_at_end(parser) && !check(parser, TOK_RBRACE)) {
    ...
    if (case_stmt) {
      ...
      // If no case statement was parsed and no error was set, break to avoid
      // infinite loop
      break;
    }
  }
...
AstNode *parse_unary(Parser *parser) {
  if (match(parser, TOK_NOT) || match(parser, TOK_BITNOT) ||
      ...
      match(parser, TOK_MULTIPLY) || match(parser, TOK_BITAND)) {
