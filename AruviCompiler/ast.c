...
void free_ast(AstNode *node) {
  ...
  // free_type(node->data_type); // Avoid double free since types are shared
  free(node);
}
...
void print_ast(AstNode *node, int indent) {
  ...
  case AST_IDENTIFIER:
    ...
    break;
  case AST_BINARY_OP:
    printf("%s", token_type_name(node->data.binary_op.op));
    break;
  case AST_TERNARY_OP:
...
// Expressions
AstNode *create_binary_op(AstNode *left, AstNode *right, TokenType op) {
...
AstNode *create_unary_op(AstNode *operand, TokenType op) {
...
AstNode *create_ternary_op(AstNode *condition, AstNode *true_expr,
                           AstNode *false_expr) {
...
AstNode *create_postfix_op(AstNode *operand, TokenType op) {
  AstNode *node = create_ast_node(AST_POSTFIX_OP); // Use specific type
  node->data.unary_op.op = op;
  add_child(node, operand);
  return node;
}

AstNode *create_assignment(AstNode *left, AstNode *right, TokenType op) {
...
AstNode *create_for_stmt(AstNode *init, AstNode *condition, AstNode *increment,
                         AstNode *body) {
...
AstNode *create_break_stmt(void) { return create_ast_node(AST_BREAK_STMT); }
...
// Declarations
AstNode *create_function_decl(const char *name, Type *return_type,
...
AstNode *create_variable_decl(const char *name, Type *type,
                              AstNode *initializer) {
...
void add_child(AstNode *parent, AstNode *child) {
  ...
  if (parent->child_count >= parent->child_capacity) {
    ...
    parent->children =
        realloc(parent->children, parent->child_capacity * sizeof(AstNode *));
  }
...
Type *create_custom_type(const char *name) {
  Type *type = create_type(
      TYPE_INT); // Default to int for now, can be improved for real typedefs
  type->name = strdup(name);
  return type;
}
